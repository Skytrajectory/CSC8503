...
bool CollisionDetection::RaySphereIntersection(const Ray& r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
    ...
    // 如果投影点与球心的距离大于球的半径，射线与球体没有相交
    if (sphereDist > sphereRadius) {
        return false;
    }

    // 计算偏移量，即射线与球的交点到投影点的距离
...
bool CollisionDetection::AABBIntersection(
	...
	if (overlap) {
		...
		// 计算两AABB之间的各个方向上的最小距离
		float distances[6] = {
			maxB.x - minA.x, // b相对于a的左侧距离
			maxA.x - minB.x, // b相对于a的右侧距离
			maxB.y - minA.y, // b相对于a的下方距离
			maxA.y - minB.y, // b相对于a的上方距离
			maxB.z - minA.z, // b相对于a的远方距离
			maxA.z - minB.z  // b相对于a的近方距离
		};
